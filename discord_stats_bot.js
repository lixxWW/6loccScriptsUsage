...
async function fetchAllTelemetry() {
    ...
    while (hasMore) {
        const fetchUrl = `${supabaseUrl}/rest/v1/telemetry?select=*&limit=${limit}&offset=${offset}`;
...
async function run() {
    try {
        ...
        rawData.forEach(row => {
            ...
            if (user && (user.toLowerCase() === "jgkjgjgh8" || user.toLowerCase() === "dontgrabme_2")) return; // Skip test accounts
        ...
        for (const [username, rows] of Object.entries(userRowsMap)) {
            rows.sort((a, b) => new Date(a.created_at || a.timestamp || 0) - new Date(b.created_at || b.timestamp || 0));
            ...
            rows.forEach(row => {
                ...
                if (playtime === 0) {
                    ...
                    const rowDate = new Date(row.created_at || row.timestamp || 0);
                    const minuteKey = rowDate.getFullYear() + "-" + 
                                      (rowDate.getMonth() + 1) + "-" + 
                                      rowDate.getDate() + " " + 
                                      rowDate.getHours() + ":" + 
                                      rowDate.getMinutes();
            ...
            if (executions === 0 && userPlaytime > 0) {
                executions = 1;
            }
        ...
        // Executor usage share calculation
        const execDistribution = {};
        usersArray.forEach(user => {
            const exec = user.executor;
            execDistribution[exec] = (execDistribution[exec] || 0) + 1;
        });

        // Sort leaderboard by playtime descending
        usersArray.sort((a, b) => b.playtime - a.playtime);

        // Generate leaderboard (Top 5)
        let leaderboardText = "";
        ...
        if (leaderboardText === "") {
            leaderboardText = "*No data recorded yet.*";
        }
        ...
        rawData.forEach(row => {
            ...
            if (rowDate.getTime() > 0) {
                ...
                if (rowDate > newestDate) newestDate = rowDate;
            }
        });
        ...
        usersArray.forEach(user => {
            totalExecutions += user.executions;
        });
        const avgDailyExecutes = (totalExecutions / daysDiff).toFixed(1);

        // Generate executor usage share dynamically
        const executorsSet = new Set(DEFAULT_EXECUTORS);
        usersArray.forEach(user => {
            if (user.executor && user.executor !== "Unknown" && user.executor !== "Unknown Executor") {
                executorsSet.add(user.executor);
            }
        });
        ...
        Array.from(executorsSet).forEach(exec => {
            ...
            // Green if share is >= 10%, otherwise orange
            const emoji = percentage >= 10 ? "🟢" : "🟠";
            execStatusText += `${emoji} **${exec}**: \`${percentage}%\`\n`;
        });

        // Build premium clean Discord Embed payload
        const embed = {
            title: "⚡ 6locc Scripts - Violence District Usage",
            description: "*Real-time statistics of active users and executor share.*",
            color: 10512895, // Purple #a069ff
            fields: [
                {
                    name: "📊 Overview",
            ...
            footer: {
                text: "Automatically updated every hour • 6locc Scripts"
            },
        ...
        if (messageId) {
            ...
            try {
                ...
                if (patchResponse.ok) {
                    ...
                    console.log(`Update failed with status: ${patchResponse.status}. Creating new message...`);
                }
            } catch (err) {
                console.error("Error during PATCH, fallback to POST:", err.message);
            }
        }

        if (!success) {
            const postUrl = `${webhookUrl}${webhookUrl.includes('?') ? '&' : '?'}wait=true`;
            console.log("Sending a new message to Discord...");
            ...
            if (!postResponse.ok) {
                ...
                throw new Error(`Discord send error: ${postResponse.status} - ${errorText}`);
            }
            ...
            console.log("Save this ID in your Github Secrets as DISCORD_MESSAGE_ID");
